...
/** Complex number represented as [real, imag] interleaved in a Float32Array. */
export interface STFTResult {
  /** Interleaved real/imag values. Shape: [numFrames, fftSize/2+1, 2] flattened. */
  data: Float32Array;
  numFrames: number;
  numBins: number; // fftSize/2 + 1
}

/**
 * Compute Short-Time Fourier Transform.
 * Returns magnitude and phase as separate arrays for easier model processing.
 */
export function computeSTFT(
  ...
  // Scratch buffers for in-place FFT
...
export function computeISTFT(
  ...
  for (let frame = 0; frame < numFrames; frame++) {
    ...
    for (let i = 0; i < fftSize; i++) {
      real[i] *= scale;
      imag[i] = 0; // discard residual imag
    }
...
/** Encode a mono or stereo audio signal to a WAV blob. */
