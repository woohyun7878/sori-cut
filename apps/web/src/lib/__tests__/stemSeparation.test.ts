  ...
  postMessage(msg: unknown) {
    ...
    if (data.type === 'separate') {
      ...
      setTimeout(() => {
        ...
        const stems = [
          { name: 'vocals', label: '보컬', channelData: [new Float32Array(length)] },
          { name: 'drums', label: '드럼', channelData: [new Float32Array(length)] },
          { name: 'bass', label: '베이스', channelData: [new Float32Array(length)] },
          { name: 'guitar', label: '기타', channelData: [new Float32Array(length)] },
        ];
