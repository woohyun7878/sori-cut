...
export interface TimelineTrack {
  ...
  /** Clip duration before the current sync alignment was applied. */
...
type ProjectUrlSlice = Pick<
  ProjectState,
  'originalAudio' | 'stems' | 'recordings' | 'video' | 'tracks'
>;
...
function rebaseTrackSync(track: TimelineTrack): TimelineTrack {
  return {
    ...track,
    syncOffset: track.startOffset - track.sourceStartOffset,
    syncBaseSourceStartOffset: track.sourceStartOffset,
    syncBaseDuration: track.duration,
  };
}

function applyTrackUpdates(track: TimelineTrack, updates: Partial<TimelineTrack>): TimelineTrack {
  const isSyncUpdate =
    updates.syncOffset !== undefined &&
    updates.syncBaseSourceStartOffset !== undefined &&
    updates.syncBaseDuration !== undefined;
  const minimumDuration = isSyncUpdate ? Math.min(0.5, track.duration) : 0.5;
  const nextTrack: TimelineTrack = {
    ...track,
    ...updates,
    duration:
      updates.duration === undefined ? track.duration : Math.max(updates.duration, minimumDuration),
    startOffset:
      updates.startOffset === undefined ? track.startOffset : Math.max(0, updates.startOffset),
    sourceStartOffset:
      updates.sourceStartOffset === undefined
        ? track.sourceStartOffset
        : Math.max(0, updates.sourceStartOffset),
  ...
  return timingChanged && !isSyncUpdate ? rebaseTrackSync(nextTrack) : nextTrack;
}
...
export const useProjectStore = create<ProjectState>()(
  undoMiddleware((set, get) => ({
      ...
      set((state) => {
        state.stems.forEach((existingStem) => {
          const stillPresent = stems.some(
            (incomingStem) =>
              incomingStem.id === existingStem.id && incomingStem.url === existingStem.url,
          );
    ...
    toggleStemMute: (id) =>
      set((state) => {
        const nextStems = state.stems.map((stem) =>
      ...
      set((state) => {
        ...
        return {
          stems: state.stems.map((stem) =>
            stem.id === id ? { ...stem, volume: nextVolume } : stem,
          ),
    ...
    addTrack: (track = {}) => {
      const fallbackDuration = Math.max(
        get().video?.duration ?? get().originalAudio?.duration ?? 8,
        4,
      );
    ...
    updateTrack: (id, updates) =>
      set((state) => ({
        tracks: state.tracks.map((track) =>
          track.id === id ? applyTrackUpdates(track, updates) : track,
        ),
      })),
    toggleTrackMute: (id) =>
      set((state) => ({
    ...
    splitTrackAtPosition: (id, position) =>
      set((state) => {
        ...
        const firstPart = rebaseTrackSync({
          ...track,
          duration: firstDuration,
        });

        const secondPart = rebaseTrackSync({
          ...track,
